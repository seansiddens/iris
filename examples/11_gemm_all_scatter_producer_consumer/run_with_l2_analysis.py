...
def display_combined_results(
    ...
    if benchmark_data:
        ...
        comm_ms = benchmark_data.get('communication_ms')
        if gemm_ms is not None:
            print(f"  • GEMM Time: {gemm_ms:.3f} ms")
        if comm_ms is not None:
            print(f"  • Communication Time: {comm_ms:.3f} ms")
        
        gemm_sms = benchmark_data.get('gemm_sms')
        comm_sms = benchmark_data.get('comm_sms')
        if gemm_sms is not None:
            print(f"\n  SM Allocation:")
            print(f"    - GEMM: {gemm_sms} SMs")
        if comm_sms is not None:
            print(f"    - Communication: {comm_sms} SMs")
...
Examples:
  ...
  ./run_with_l2_analysis.py --profile-l2 --variant baseline -m 8192 -n 4608 -k 36864
  
  # Run spatial variant with benchmarking
  ./run_with_l2_analysis.py --profile-l2 --variant spatial --benchmark
  
  # Run with validation and show XCD maps
  ./run_with_l2_analysis.py --validate --show_map
        """
    )
    ...
    parser.add_argument("--variant", type=str, choices=["baseline", "spatial"], 
    ...
    parser.add_argument("--comm_sms", type=int, help="Number of SMs for communication")
    ...
    if args.variant:
        benchmark_args.extend(["--variant", args.variant])
    if args.benchmark:
        benchmark_args.append("--benchmark")
    if args.validate:
        benchmark_args.append("--validate")
    if args.show_map:
        benchmark_args.append("--show_map")
    ...
    if args.comm_sms is not None:
        benchmark_args.extend(["--comm_sms", str(args.comm_sms)])
    ...
    variant = args.variant if args.variant else benchmark_data.get('variant', 'unknown') if benchmark_data else 'unknown'
